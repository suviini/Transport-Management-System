package publicTransport;

import java.awt.EventQueue;
import javax.swing.JFrame;
import javax.swing.JLabel;
import javax.swing.JTextField;
import javax.swing.JButton;
import javax.swing.JPasswordField;
import javax.swing.JOptionPane;
import java.awt.Font;
import java.awt.Color;
import java.awt.event.ActionListener;
import java.awt.event.ActionEvent;
import java.sql.Connection;
import java.sql.PreparedStatement;
import java.sql.ResultSet;
import javax.swing.JPanel;
import javax.swing.ImageIcon;
import transport.home; 

public class Login {

    private JFrame frame;
    private JPasswordField passwordField;
    private JTextField usernameField;

    public static void main(String[] args) {
        EventQueue.invokeLater(new Runnable() {
            public void run() {
                try {
                    // Clear any existing session
                    Session.clear();
                    
                    Login window = new Login();
                    window.frame.setVisible(true);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
        });
    }

    public Login() {
        initialize();
    }

    private void initialize() {
        frame = new JFrame();
        frame.getContentPane().setBackground(new Color(255, 255, 255));
        frame.setBounds(100, 100, 1113, 776);
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        frame.getContentPane().setLayout(null);
        
        // Disable maximize and full screen
        frame.setResizable(false); // Disable resizing the window
        frame.setExtendedState(JFrame.NORMAL); // Set to normal mode, not maximized
        
        JPanel panel = new JPanel();
        panel.setBackground(new Color(254, 222, 73));
        panel.setBounds(0, 0, 435, 858);
        frame.getContentPane().add(panel);
        panel.setLayout(null);
        
        JLabel lblCustomerLogin = new JLabel("Welcome");
        lblCustomerLogin.setBounds(138, 131, 239, 47);
        panel.add(lblCustomerLogin);
        lblCustomerLogin.setForeground(new Color(0, 0, 0));
        lblCustomerLogin.setFont(new Font("Georgia", Font.BOLD, 32));

        JLabel lblUsername = new JLabel("Username ");
        lblUsername.setBounds(52, 274, 100, 26);
        panel.add(lblUsername);
        lblUsername.setForeground(new Color(0, 0, 0));
        lblUsername.setFont(new Font("Tahoma", Font.BOLD, 16));

        usernameField = new JTextField();
        usernameField.setBackground(new Color(254, 222, 73));
        usernameField.setBounds(183, 275, 194, 30);
        panel.add(usernameField);
        usernameField.setColumns(10);

        JLabel lblPassword = new JLabel("Password");
        lblPassword.setBounds(52, 361, 100, 20);
        panel.add(lblPassword);
        lblPassword.setForeground(new Color(0, 0, 0));
        lblPassword.setFont(new Font("Tahoma", Font.BOLD, 16));

        passwordField = new JPasswordField();
        passwordField.setBackground(new Color(254, 222, 73));
        passwordField.setBounds(183, 359, 194, 30);
        panel.add(passwordField);

        // Login Button
        JButton btnLogin = new JButton("Login");
        btnLogin.setForeground(new Color(255, 255, 255));
        btnLogin.setBackground(new Color(0, 0, 0));
        btnLogin.setBounds(168, 439, 116, 39);
        panel.add(btnLogin);
        btnLogin.setFont(new Font("Tahoma", Font.BOLD, 15));

        // Sign Up Button (added to redirect users to the signup form)
        JButton btnSignup = new JButton("Sign Up");
        btnSignup.setForeground(new Color(255, 255, 255));
        btnSignup.setBackground(new Color(0, 0, 0));
        btnSignup.setBounds(302, 515, 100, 39);
        panel.add(btnSignup);
        btnSignup.setFont(new Font("Tahoma", Font.BOLD, 15));
        
        JLabel lblLoginToYour = new JLabel("Login to your account to continue");
        lblLoginToYour.setForeground(Color.BLACK);
        lblLoginToYour.setFont(new Font("Georgia", Font.PLAIN, 20));
        lblLoginToYour.setBounds(62, 181, 443, 26);
        panel.add(lblLoginToYour);
        
        JLabel lblDontYouHave = new JLabel("Don't you have an account?");
        lblDontYouHave.setForeground(Color.BLACK);
        lblDontYouHave.setFont(new Font("Georgia", Font.PLAIN, 20));
        lblDontYouHave.setBounds(26, 520, 267, 26);
        panel.add(lblDontYouHave);
        
        JLabel lblNewLabel = new JLabel("New label");
        lblNewLabel.setIcon(new ImageIcon(Login.class.getResource("/Icons/ulogin.jpg")));
        lblNewLabel.setBounds(283, 114, 847, 522);
        frame.getContentPane().add(lblNewLabel);
        
        // Action listener for Sign Up button
        btnSignup.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                Signup.main(null); // Open Signup Page
                frame.dispose(); // Close the login frame
            }
        });

        // Action listener for Login button
        btnLogin.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                String username = usernameField.getText().trim();
                String password = new String(passwordField.getPassword()).trim();

                // Check for empty fields
                if (username.isEmpty() || password.isEmpty()) {
                    JOptionPane.showMessageDialog(frame, "Please enter both username and password", "Validation Error", JOptionPane.WARNING_MESSAGE);
                } else {
                    User user = validateLogin(username, password);
                    if (user != null) {
                        // Successful login, set user in session and open HomePage frame
                        Session.setCurrentUser(user);
                        JOptionPane.showMessageDialog(frame, "Login successful! Welcome, " + user.getName() + ".", "Success", JOptionPane.INFORMATION_MESSAGE);
                        
                        // Open HomePage frame
                        home homePage = new home(); // Create instance of HomePage
                        homePage.setVisible(true); // Show HomePage
                        frame.dispose(); // Close the login frame
                    } else {
                        // Show error message
                        JOptionPane.showMessageDialog(frame, "Incorrect credentials", "Login Error", JOptionPane.ERROR_MESSAGE);
                        usernameField.setText(""); 
                        passwordField.setText(""); 
                    }
                }
            }
        });
    }

    private User validateLogin(final String username, final String password) {
        User user = null;
        Connection conn = null;
        PreparedStatement stmt = null;
        ResultSet rs = null;
        try {
            conn = DatabaseConnection.getConnection();
            String sql = "SELECT user_id, name, username FROM users WHERE username = ? AND password = ?";
            stmt = conn.prepareStatement(sql);
            stmt.setString(1, username);
            stmt.setString(2, password);

            rs = stmt.executeQuery();

            // If a record is found, credentials are valid
            if (rs.next()) {
                int userId = rs.getInt("user_id");
                String name = rs.getString("name");
                String uname = rs.getString("username");
                user = new User(userId, name, uname);
            }
        } catch (Exception ex) {
            ex.printStackTrace();
            JOptionPane.showMessageDialog(frame, "An error occurred during login: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        } finally {
            try {
                if (rs != null) rs.close();
                if (stmt != null) stmt.close();
                if (conn != null) conn.close();
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        return user;
    }

    public void setVisible(boolean b) {
        frame.setVisible(b);
    }
}